module.exports = async function(payload,config,nodeconfig,meterinfo,storage) {
  ...
  try {
          const retrieveAndSet = async function(statname,nodeconfig,config,meterinfo,resolution,from,to,readings,prodReadings) {
            ...
            console.log('fetchTimeFrame',statname);
      ...
      if((typeof payload.stats.gsi == "undefined")||(payload.stats.gsi.updated==null)||(payload.stats.gsi.updated < new Date().getTime() - (900000*1))) {
          if((typeof meterinfo.location !== 'undefined') && (typeof meterinfo.location.zip !== 'undefined')) {
            ...
            let energyOut = Math.abs(Math.round(energyProd - (payload.stats.last24h.energySelf_wh * 10000000 * efactor)));
            let energy = (payload.stats.last24h.consumption_wh * 10000000) - (payload.stats.last24h.energySelf_wh * 10000000 * efactor);
            readings.push({
