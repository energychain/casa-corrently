module.exports = async function(msg,config,storage,RED) {
    ...
    if((typeof node.config.credentials.username == 'undefined') || (node.config.credentials.username == null) || (node.config.credentials.username.length < 3)) {
      ...
      msg.payload.fingerprint = {
        config: crypto.createHash('sha256').update(JSON.stringify(config)).digest('base64'),
        system: crypto.createHash('sha256').update(JSON.stringify(system_fp)).digest('base64')
      }
      return msg.payload;
};
